...
def sentiwordnet(number_classes, train, dev, test) -> (pd.Series, pd.Series, pd.Series):
    ...
    # x_train = train['Phrase'].apply(get_sentiwordnet_scores)
    # y_train = train[number_classes]
    # x_dev = dev['Phrase'].apply(get_sentiwordnet_scores)
    # x_test = test['Phrase'].apply(get_sentiwordnet_scores)
    #
    # return x_train, y_train, x_dev, x_test

    train_sentences = train['Phrase']
    dev_sentences = dev['Phrase']
